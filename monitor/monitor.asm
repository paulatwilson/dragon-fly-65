...
DISASM_NOT_CMP_IMM:
    cmp     #$E0
    bne     DISASM_NOT_CPX_IMM
    jmp     DISASM_CPX_IMM
DISASM_NOT_CPX_IMM:
    cmp     #$C0
    bne     DISASM_NOT_CPY_IMM
    jmp     DISASM_CPY_IMM
DISASM_NOT_CPY_IMM:
    cmp     #$89
    bne     DISASM_NOT_BIT_IMM
    jmp     DISASM_BIT_IMM
DISASM_NOT_BIT_IMM:
    cmp     #$29
...
DISASM_NOT_CPX_DP:
    cmp     #$EC
    bne     DISASM_NOT_CPX_ABS
    jmp     DISASM_CPX_ABS
DISASM_NOT_CPX_ABS:
    cmp     #$C4
    bne     DISASM_NOT_CPY_DP
    jmp     DISASM_CPY_DP
DISASM_NOT_CPY_DP:
    cmp     #$CC
    bne     DISASM_NOT_CPY_ABS
    jmp     DISASM_CPY_ABS
DISASM_NOT_CPY_ABS:
    cmp     #$24
    bne     DISASM_NOT_BIT_DP
    jmp     DISASM_BIT_DP
DISASM_NOT_BIT_DP:
    cmp     #$34
    bne     DISASM_NOT_BIT_DPX
    jmp     DISASM_BIT_DPX
DISASM_NOT_BIT_DPX:
    cmp     #$2C
    bne     DISASM_NOT_BIT_ABS
    jmp     DISASM_BIT_ABS
DISASM_NOT_BIT_ABS:
    cmp     #$3C
    bne     DISASM_NOT_BIT_ABSX
    jmp     DISASM_BIT_ABSX
DISASM_NOT_BIT_ABSX:
    cmp     #$1A
    bne     DISASM_NOT_INC_A
    jmp     DISASM_INC_A
DISASM_NOT_INC_A:
    cmp     #$E6
    bne     DISASM_NOT_INC_DP
    jmp     DISASM_INC_DP
DISASM_NOT_INC_DP:
    cmp     #$F6
    bne     DISASM_NOT_INC_DPX
    jmp     DISASM_INC_DPX
DISASM_NOT_INC_DPX:
    cmp     #$EE
    bne     DISASM_NOT_INC_ABS
    jmp     DISASM_INC_ABS
DISASM_NOT_INC_ABS:
    cmp     #$FE
    bne     DISASM_NOT_INC_ABSX
    jmp     DISASM_INC_ABSX
DISASM_NOT_INC_ABSX:
    cmp     #$3A
    bne     DISASM_NOT_DEC_A
    jmp     DISASM_DEC_A
DISASM_NOT_DEC_A:
    cmp     #$C6
    bne     DISASM_NOT_DEC_DP
    jmp     DISASM_DEC_DP
DISASM_NOT_DEC_DP:
    cmp     #$D6
    bne     DISASM_NOT_DEC_DPX
    jmp     DISASM_DEC_DPX
DISASM_NOT_DEC_DPX:
    cmp     #$CE
    bne     DISASM_NOT_DEC_ABS
    jmp     DISASM_DEC_ABS
DISASM_NOT_DEC_ABS:
    cmp     #$DE
...
ASM_PARSE_DEC_OK:
    ...
    beq     ASM_PARSE_DEC_A
    cmp     #0
    beq     ASM_PARSE_DEC_DP
    cmp     #1
    beq     ASM_PARSE_DEC_ABS
    cmp     #2
    beq     ASM_PARSE_DEC_DPX
    cmp     #4
    beq     ASM_PARSE_DEC_ABSX
    jmp     ASM_FAIL
ASM_PARSE_DEC_A:
    lda     #$3A                ; DEC accumulator
    jmp     ASM_EMIT_IMPLIED
ASM_PARSE_DEC_DP:
    lda     #$C6                ; DEC dp
    jsr     ASM_EMIT_A
...
ASM_PARSE_B:
    jsr     ASM_READ_UPPER
    cmp     #'E'
    bne     ASM_PARSE_B_NOT_E
    jmp     ASM_PARSE_BEQ
ASM_PARSE_B_NOT_E:
    cmp     #'N'
    bne     ASM_PARSE_B_NOT_N
    jmp     ASM_PARSE_BNE
ASM_PARSE_B_NOT_N:
    cmp     #'C'
    bne     ASM_PARSE_B_NOT_C
    jmp     ASM_PARSE_BC
ASM_PARSE_B_NOT_C:
    cmp     #'M'
    bne     ASM_PARSE_B_NOT_M
    jmp     ASM_PARSE_BMI
ASM_PARSE_B_NOT_M:
    cmp     #'P'
...
ASM_PARSE_BIT_ADDR:
    ...
    lda     ZP_ERR
    beq     ASM_PARSE_BIT_ADDR_OK
    rts
ASM_PARSE_BIT_ADDR_OK:
    lda     ZP_TMP2
    cmp     #0
    beq     ASM_PARSE_BIT_DP
    cmp     #1
    beq     ASM_PARSE_BIT_ABS
    cmp     #2
    beq     ASM_PARSE_BIT_DPX
    cmp     #4
    beq     ASM_PARSE_BIT_ABSX
    jmp     ASM_FAIL
ASM_PARSE_BIT_DP:
    lda     #$24                ; BIT dp
    jsr     ASM_EMIT_A
...
ASM_PARSE_CP:
    ...
    jmp     ASM_FAIL
ASM_PARSE_CPX:
    jsr     ASM_PARSE_CP_OPER
    lda     ZP_ERR
    beq     ASM_PARSE_CPX_OK
    rts
ASM_PARSE_CPX_OK:
    lda     ZP_TMP2
    cmp     #6
    beq     ASM_PARSE_CPX_IMM
    cmp     #0
    beq     ASM_PARSE_CPX_DP
    cmp     #1
...
ASM_PARSE_CPY:
    jsr     ASM_PARSE_CP_OPER
    lda     ZP_ERR
    beq     ASM_PARSE_CPY_OK
    rts
ASM_PARSE_CPY_OK:
    lda     ZP_TMP2
    cmp     #6
    beq     ASM_PARSE_CPY_IMM
    cmp     #0
    beq     ASM_PARSE_CPY_DP
    cmp     #1
...
ASM_PARSE_CP_OPER:
    ...
    rts
ASM_PARSE_CP_IMM_OK:
    lda     #6                  ; immediate marker for caller
    sta     ZP_TMP2
    rts
ASM_PARSE_CP_ADDR:
    jsr     ASM_PARSE_ADDR_OPER
    lda     ZP_ERR
    beq     ASM_PARSE_CP_ADDR_OK
    rts
ASM_PARSE_CP_ADDR_OK:
    lda     ZP_TMP2
    cmp     #0
    beq     ASM_PARSE_CP_ADDR_DONE
    cmp     #1
    beq     ASM_PARSE_CP_ADDR_DONE
    jmp     ASM_FAIL
ASM_PARSE_CP_ADDR_DONE:
    rts

ASM_PARSE_INCDEC_OPER:
    jsr     ASM_SKIP_SPACES
    jsr     ASM_X_AT_EOL
    cmp     #1
    beq     ASM_PARSE_INCDEC_ACC
    lda     $0000,x
    jsr     ASM_UPPER_A
    cmp     #'A'
    bne     ASM_PARSE_INCDEC_ADDR
    inx
    jsr     ASM_SKIP_SPACES
    jsr     ASM_X_AT_EOL
    cmp     #1
    beq     ASM_PARSE_INCDEC_ACC
    jmp     ASM_FAIL
ASM_PARSE_INCDEC_ACC:
    lda     #6                  ; accumulator marker for caller
    sta     ZP_TMP2
    rts
ASM_PARSE_INCDEC_ADDR:
    jsr     ASM_PARSE_ADDR_OPER
    lda     ZP_ERR
    beq     ASM_PARSE_INCDEC_ADDR_OK
    rts
ASM_PARSE_INCDEC_ADDR_OK:
    lda     ZP_TMP2
    cmp     #0
    beq     ASM_PARSE_INCDEC_ADDR_DONE
    cmp     #1
    beq     ASM_PARSE_INCDEC_ADDR_DONE
    cmp     #2
    beq     ASM_PARSE_INCDEC_ADDR_DONE
...
ASM_PARSE_INC_OK:
    ...
    beq     ASM_PARSE_INC_A
    cmp     #0
    beq     ASM_PARSE_INC_DP
    cmp     #1
    beq     ASM_PARSE_INC_ABS
    cmp     #2
    beq     ASM_PARSE_INC_DPX
    cmp     #4
    beq     ASM_PARSE_INC_ABSX
    jmp     ASM_FAIL
ASM_PARSE_INC_A:
    lda     #$1A                ; INC accumulator
    jmp     ASM_EMIT_IMPLIED
ASM_PARSE_INC_DP:
    lda     #$E6                ; INC dp
    jsr     ASM_EMIT_A
